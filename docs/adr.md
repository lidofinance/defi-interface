...
### Read-Only Reentrancy via Target Vault (Integration, Critical)
- **Threat:** Adapters call `TARGET_VAULT.convertToAssets()` to calculate `totalAssets()`, which is used during deposits to determine share minting amounts. If the target vault relies on manipulable price sources (e.g., Curve pool's `get_virtual_price()` during flash loan callbacks), an attacker can inflate or deflate the reported asset value. A deflated value causes the vault to mint excess shares to the attacker, who can later redeem at true value for profit. The base vault's `nonReentrant` modifier does NOT protect against this because the attack manipulates external state (Curve pool) rather than reentering the same contract.
- **Mitigation:** Protocol-specific adapters (e.g., `CurveAdapter`) override `deposit()`, `mint()`, and optionally `totalAssets()` to add manipulation checks before calling parent logic. Strategies include: (1) detecting abnormal price deviations within the same block (e.g., >5% swing indicates manipulation), (2) using protocol-native reentrancy locks (e.g., Curve's `is_killed()` or lock flags), (3) caching price values and rejecting suspicious intra-block changes, (4) implementing TWAP (Time-Weighted Average Price) for high-risk integrations. All protections live in the adapter layer—**Vault and EmergencyVault remain untouched** as they are under audit. Before deploying any adapter, its target vault must be audited for read-only reentrancy vectors; only vaults with safe pricing mechanisms (Chainlink oracles, internal accounting, non-manipulable sources) or adapters with adequate guards should be approved for production.
- **Residual risk:** Adapter-level mitigations depend on correct implementation and may not catch sophisticated multi-block or cross-protocol manipulation; zero-day vulnerabilities in target vaults remain possible. Maintaining a whitelist of audited target vaults and continuous monitoring of integrated protocols are essential operational controls.

### Instant Rewards & Deposit Fees Diluting Fee Logic (Economic, Medium)
- **Threat:** Some target vaults auto-claim rewards or charge entry fees during our `deposit()` flow. If we naïvely set `lastTotalAssets = totalAssets()` after deposit, those deltas would be treated as principal: instant rewards could dodge performance fees and entry fees would dilute all holders.
- **Mitigation:** `Vault.deposit()` measures the actual asset delta realized after `_depositToProtocol()` (comparing `totalAssets` before/after) and mints shares from that credited amount. `lastTotalAssets` is only advanced to the post-deposit total, so instant rewards remain above the high-water mark (feeable on the next `_harvestFees()`) and entry fees/non-ideal fills are borne by the initiating depositor via fewer shares minted.
- **Residual risk:** Withdrawals still rely on `_withdrawFromProtocol()` returning the requested assets; exit fees or slippage remain socialized unless adapters add protocol-specific accounting.
